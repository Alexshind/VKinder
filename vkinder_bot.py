...
def bot(user_token, public_token, db_user_name='postgres', db_password='your pass', db='your db', memory_days=0):
    ...
    def __get_photos_args(list_of_photo):
        photos_dict = {f'photo_{number + 1}': photo for number,
                       photo in enumerate(list_of_photo)}
        return photos_dict
    ...
    def send_next_person(copy_person=True):
        ...
        if person:
            person_id, first_name, surname, link, photos = content_generator(
                person, vk_me)
            if copy_person:
    ...
    def start(vk_me, user_sex=None, user_birth_year=None, user_city_title=None):
        ...
        if not db.get_next_person(user_id, check=True):
            ...
            if not additional_age:
                ...
                write_msg(
                    user_id, 'Новых анкет нет, но Вы можете начать заново!')
                return
            db.add_find_people(str(user_id), find_people(
                user_sex, user_birth_year, user_city_title, vk_me))
    ...
    def open_favorites(user_id):
        ...
        if favorites:
            ...
            write_msg(user_id, f'Ваше избранное:')
            ...
            for event in longpoll.listen():
                ...
                if event.type == VkEventType.USER_TYPING:
                    if position == Position.INTRO:
                        write_msg(
                            user_id, 'Привет! Для поиска пары нажмите кнопку "Старт"')
                    ...
                    if event.to_me:
                        ...
                        if position == Position.INTRO and request.capitalize() == Command.START:
                            ...
                            if time() - date > (memory_days * 86400) if memory_days else -1:
                                ...
                                additional_ages = __get_additional_ages()
                                db.update_user(
                                    user_id, age_range=additional_ages, birthday=0, city='', date=time())

                            if not any([birth_year, city]):
                                user_sex, user_birth_year, user_city_title = get_user_info(
                                    user_id, vk_me)
                            ...
                            db.update_user(user_id, birthday=int(
                                datetime.now().year - int(request)))
                            ...
                            try:
                                adapted_city, etalon_city = get_city_list(
                                    r'путь к файлу:cities.json')
                                index = adapted_city.index(
                                    request.strip().lower().replace('-', ' '))
                                db.update_user(
                                    user_id, city=etalon_city[index])
                                write_msg(user_id, 'Принято!')
                                ...
                                write_msg(
                                    user_id, 'Неверный ввод! Введите Ваш город')
                            ...
                            write_msg(user_id, 'Всего доброго! Успехов!')
                            ...
                            last_send_person_info, last_send_person_photos, last_id = db.get_last_send_person(
                                user_id)
                            ...
                            write_msg(
                                user_id, 'Введите ID пользователя для удаления')
                            ...
                            db.update_user(user_id, position=3)
                            write_msg(
                                user_id, f'Пользователь с id {request} успешно удалён')
                            open_favorites(user_id)
